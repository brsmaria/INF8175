...
class MyPlayer(PlayerHex):
    ...
    def compute_action(
        ...
        depth = self.get_depth_for_time(remaining_time)

        return self.minimax_search(current_state, current_hash, depth)
    ...
    def get_depth_for_time(self, remaining_time: int) -> int:
        """
        Determines the search depth based on remaining time.
        
        Args:
            remaining_time (int): Remaining time in seconds
            
        Returns:
            int: Depth to use for minimax search
        """
        # Convert to minutes for easier thresholds
        minutes_left = remaining_time
        
        if minutes_left <= 90:  # Less than 90 seconds (1.5 minutes)
            return 2
        elif minutes_left <= 180:  # Between 90-180 seconds (1.5-3 minutes)
            return 3
        elif minutes_left <= 300:  # Between 180-300 seconds (3-5 minutes)
    ...
    def minimax_search(self, current_state: GameState, current_hash: int, depth: int = None) -> Action:
        ...
        _, best_action = self.max_value(
            current_state, depth, float("-inf"), float("inf"), current_hash
        )
